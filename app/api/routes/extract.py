    ...
    if ct == "application/pdf":
        try:
            ...
            if msg.startswith("INVALID_PDF"):
                raise InvalidInput("Invalid or corrupted PDF.")
            if msg == "ENCRYPTED_PDF":
                raise InvalidInput("Encrypted PDF is not supported.")
            if msg == "PDF_TOO_MANY_PAGES":
                raise PayloadTooLarge("PDF exceeds maximum allowed page count.")
            raise InternalError("Extraction failed unexpectedly.")
    ...
    if ct in ("image/png", "image/jpeg", "image/tiff"):
        try:
            ...
            if msg == "IMAGE_TOO_LARGE":
                raise PayloadTooLarge("Image too large to OCR safely.")
            raise InternalError("OCR failed unexpectedly.")
