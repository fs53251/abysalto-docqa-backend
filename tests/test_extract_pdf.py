...
def test_extract_pdf_text_creates_text_json(
    client: TestClient,
    temp_data_dir: Path,
    monkeypatch,
):
    ...
    pdf_bytes = make_pdf_bytes_with_text(
        "Hello PyMuPDF - this is long enough to avoid OCR fallback."
    )
    ...
    r2 = client.post(
        f"/documents/{doc_id}/extract-text", params={"ocr_fallback": False}
    )
...
def test_extract_marks_empty_pages(
    client: TestClient,
    temp_data_dir: Path,
    monkeypatch,
):
...
def test_extract_rejects_bad_doc_id(client: TestClient, temp_data_dir: Path):
